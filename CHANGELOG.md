...
- A plugin whose `setup()` throws now also reports to the console outside production. The `error` event still fires with the same payload, but it fires before `createVList` returns, so no listener could hear it and a half-wired list came back silently.
