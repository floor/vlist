...
// ── items wins over a wider grouping callback too ────────────────────────────
//
...
const widerGroups = createVListFromConfig({
  ...
  items,
  item: { height: 40, template: (entity: Entity) => entity.name },
  groups: { getGroupForIndex: (_index, entity?: Entity) => entity?.name ?? "" },
});
const widerGroupsItem: Row | undefined = widerGroups.getItemAt(0);
void widerGroupsItem;
// An Entity is not a Row: it has no `category`, and the list must refuse it.
// @ts-expect-error the list holds Row, whatever its callbacks were annotated with
widerGroups.setItems([{ id: 2, name: "b" }]);

const widerGroupsNamedTemplate = createVListFromConfig({
  container,
  items,
  item: { height: 40, template: entityTemplate },
  groups: { getGroupForIndex: (_index, entity?: Entity) => entity?.name ?? "" },
});
const widerGroupsNamedTemplateItem: Row | undefined = widerGroupsNamedTemplate.getItemAt(0);
void widerGroupsNamedTemplateItem;

// The same configuration with the grouping config in a typed variable: how an
// equivalent config is written must not change what the list holds.
const entityGroups: GroupsConfig<Entity> = { getGroupForIndex: (_index, entity) => entity?.name ?? "" };
const widerGroupsNamedConfig = createVListFromConfig({ container, items, item, groups: entityGroups });
const widerGroupsNamedConfigItem: Row | undefined = widerGroupsNamedConfig.getItemAt(0);
void widerGroupsNamedConfigItem;
