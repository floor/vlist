...
for(const [entry,create] of carouselEntries) for(const direction of [1,-1]) for(const accumulated of [false,true]) {
 test(`${entry} ${direction>0?'next':'prev'} preserves ${accumulated?'multi-lap':'single-item'} pending target across a fold`,()=>{
  ...
  try {
   ...
   // The position lives in the home lap, [1000, 2000), whichever way it left it:
   // a fold is symmetric, so `prev` from item 0 rests at 1899, never at 899.
   const inHomeLap=(offset:number)=>1000+((offset%1000)+1000)%1000;
   const start=inHomeLap(direction*foldedCount*100+direction);
   ...
   const target=inHomeLap(direction*(foldedCount+1)*100);
