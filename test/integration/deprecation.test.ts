...
it("deprecated external scroll hooks still install a writer and disable viewport input", () => {
  ...
  try {
    ...
    // On next the getter is unused (#146): sources commit through commitScroll,
    // so a deprecated writer that does not commit leaves the cached position alone.
    expect(list.getScrollPosition()).toBe(0);
    ctx.commitScroll(200);
