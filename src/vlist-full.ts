...
export const createVList = <T extends VListItem = VListItem>(
  ...
  const {
    ...
    scroll: scrollConfig,
    scrollbar: legacyScrollbarConfig,
    loading: loadingConfig,
    idleTimeout: legacyIdleTimeout,
    classPrefix = DEFAULT_CLASS_PREFIX,
    scrollElement: legacyScrollElement,
    ariaLabel,
    groups: groupsConfig,
  ...
  // ---------------------------------------------------------------------------
  // Resolve scroll config (new scroll.* with legacy top-level fallbacks)
  // ---------------------------------------------------------------------------
  ...
  // Resolve scrollbar mode: 'custom' | 'native' | 'none'
  // and extract scrollbar options for the custom scrollbar
  type ScrollbarMode = "custom" | "native" | "none";
  let scrollbarMode: ScrollbarMode;
  ...
  if (scrollConfig?.scrollbar !== undefined) {
    // New API: scroll.scrollbar
    if (scrollConfig.scrollbar === "native") {
      scrollbarMode = "native";
    } else if (scrollConfig.scrollbar === "none") {
      scrollbarMode = "none";
    } else {
      // Object form — custom scrollbar with options
      scrollbarMode = "custom";
      scrollbarOptions = scrollConfig.scrollbar;
    }
  } else if (legacyScrollbarConfig !== undefined) {
    // Legacy API: top-level scrollbar config
    if (legacyScrollbarConfig.enabled === false) {
      scrollbarMode = "none";
    } else {
      scrollbarMode = "custom";
      scrollbarOptions = legacyScrollbarConfig;
    }
  } else {
  ...
  // Store reference to current grid state for dynamic height calculation
  let currentGridState = {
    containerWidth: dom.viewport.clientWidth,
    columns: gridLayout?.columns ?? 0,
    gap: gridLayout?.gap ?? 0,
  };

  // In grid mode, wrap height function to inject grid context
  // This allows height to be calculated based on column width for aspect ratios
  if (isGrid && gridLayout && typeof effectiveHeightConfig === "function") {
    const baseFn = effectiveHeightConfig;

    effectiveHeightConfig = (index: number) => {
      // Always use current grid state (not captured in closure)
      const innerWidth = currentGridState.containerWidth - 2; // account for borders
      const totalGaps = (currentGridState.columns - 1) * currentGridState.gap;
      const columnWidth = (innerWidth - totalGaps) / currentGridState.columns;

      const context: import("./types").GridHeightContext = {
        containerWidth: currentGridState.containerWidth,
        columns: currentGridState.columns,
        gap: currentGridState.gap,
        columnWidth,
      };

      // Call user's function with context
      const height = baseFn(index, context);
      return height + currentGridState.gap; // Add gap for row spacing
    };
  } else {
  ...
  // Scrollbar setup
  // ---------------------------------------------------------------------------
  // Custom scrollbar is the DEFAULT. Native is only used when explicitly requested
  // AND compression is not active. In window mode, no custom scrollbar is created
  // (the browser's page-level scrollbar is used).
  //
  // The CSS class `vlist-viewport--custom-scrollbar` hides the native scrollbar
  // via `scrollbar-width: none` while keeping `overflow: auto` for programmatic
  // scroll support.

  // Determine if custom scrollbar should be created right now
  const shouldCreateCustomScrollbar =
    !isWindowMode &&
    scrollbarMode !== "none" &&
    (scrollbarMode === "custom" ||
      (scrollbarMode === "native" && initialCompression.isCompressed));

  let scrollbar = shouldCreateCustomScrollbar
    ? createScrollbar(
        dom.viewport,
  ...
  // - Custom scrollbar is active (avoid double scrollbar)
  // - scrollbar mode is 'none' (no scrollbar at all)
  // - wheel is false (prevent scrollbar-drag scrolling)
  const shouldHideNativeScrollbar =
    !isWindowMode &&
    (scrollbarMode !== "native" ||
      initialCompression.isCompressed ||
      !wheelEnabled);

  if (shouldHideNativeScrollbar) {
  ...
  const updateCompressionMode = (): void => {
    ...
    if (compression.isCompressed && !scrollController.isCompressed()) {
      ...
      // Create custom scrollbar if not exists and mode allows it.
      // When scrollbar mode is 'native', compression forces a fallback to custom.
      // When mode is 'none', never create a scrollbar.
      const shouldCreateOnCompress =
        !isWindowMode && !scrollbar && scrollbarMode !== "none";
      if (shouldCreateOnCompress) {
  ...
  const update = (updateConfig: import("./types").VListUpdateConfig): void => {
    if (ctx.state.isDestroyed) {
      throw new Error("[vlist] Cannot update destroyed instance");
    }

    let needsRerender = false;

    // Update grid configuration
    if (updateConfig.grid && isGrid && gridRenderer) {
      const { columns, gap } = updateConfig.grid;

      if (columns !== undefined) {
        if (!Number.isInteger(columns) || columns < 1) {
          throw new Error(
            "[vlist] grid.columns must be a positive integer >= 1",
          );
        }
      ...
      if (gap !== undefined) {
        if (gap < 0) {
          throw new Error("[vlist] grid.gap must be non-negative");
        }
        gridConfig.gap = gap;
        needsRerender = true;
      }
      ...
      if (needsRerender) {
        // Update grid layout with new columns/gap (no recreation needed)
        if (gridLayout) {
          gridLayout.update(gridConfig);
        }

        // Update grid renderer with new container width
        const containerWidth = dom.viewport.clientWidth;
        if (gridRenderer) {
          gridRenderer.updateContainerWidth(containerWidth);
        }

        // Update grid state reference if height function is dynamic
        if (typeof mainAxisSizeConfig === "function") {
          // Update the grid state that the height function references
          currentGridState.containerWidth = containerWidth;
          currentGridState.columns = gridConfig.columns;
          currentGridState.gap = gridConfig.gap;

          // Recalculate total rows
          const totalRows = Math.ceil(
            dataManager.getTotal() / gridConfig.columns,
          );

          // Rebuild height cache (it will call the function with updated state)
          heightCache.rebuild(totalRows);

          // Recalculate total height
          const totalHeight = heightCache.getTotalHeight();
          ctx.state.viewportState.totalHeight = totalHeight;
          updateContentSize(totalHeight);
        }

        // Recalculate viewport (row range changed due to new columns)
        updateViewport();

        // Clear and force full re-render with new layout
        if (gridRenderer) {
          gridRenderer.clear();
        }
        forceRender();
    ...
    // Update item height (used when grid columns change to maintain aspect ratio)
    if (updateConfig.itemHeight !== undefined) {
      const newHeight = updateConfig.itemHeight;

      // Update config
      (ctx.config as any).itemHeight = newHeight;

      // For grid mode, add gap to height
      const effectiveHeight =
        isGrid && gridLayout
          ? typeof newHeight === "function"
            ? (index: number) =>
                (newHeight as (i: number) => number)(index) + gridLayout.gap
            : newHeight + gridLayout.gap
          : newHeight;

      // Get current row count (for grid) or item count (for list)
      const totalCount =
        isGrid && gridLayout
          ? gridLayout.getTotalRows(dataManager.getTotal())
          : dataManager.getTotal();

      // Recreate height cache with new height
      const newHeightCache = createHeightCache(effectiveHeight, totalCount);
      (ctx as any).heightCache = newHeightCache;

      // Update renderer's height cache reference if it exists
      if (renderer && (renderer as any).heightCache) {
        (renderer as any).heightCache = newHeightCache;
      }

      // Recalculate total height
      const totalHeight = newHeightCache.getTotalHeight();
      ctx.state.viewportState.totalHeight = totalHeight;
      updateContentSize(totalHeight);
    ...
    if (updateConfig.selectionMode !== undefined) {
      const newMode = updateConfig.selectionMode;
      if (newMode !== ctx.config.selectionMode) {
        // Update selection mode
        (ctx.config as any).selectionMode = newMode;

        // Clear selection if switching to "none"
        if (newMode === "none") {
          selectionMethods.clearSelection();
        }
        // Clear multi-selection if switching to "single"
        else if (newMode === "single") {
          const selected = Array.from(ctx.state.selectionState.selected);
          if (selected.length > 1) {
            // Keep only the first selected item
            ctx.state.selectionState.selected.clear();
            ctx.state.selectionState.selected.add(selected[0]);
          }
        }
      }
    }
    ...
    if (updateConfig.overscan !== undefined) {
      if (updateConfig.overscan < 0) {
        throw new Error("[vlist] overscan must be non-negative");
      }
      (ctx.config as any).overscan = updateConfig.overscan;
      needsRerender = true;
    }
